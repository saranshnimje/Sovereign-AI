...
# Poll interval when waiting for a decision (seconds)
_POLL_INTERVAL = 2.0
...
class ApprovalService:
    ...
    async def wait_for_decision(self, request_id: str) -> tuple[bool, str | None]:
        """
        Poll the DB until the request is approved, rejected, or expired.
        Returns (approved: bool, decision_note: str|None).

        This runs inside the agent execution task.
        It yields to the event loop while waiting so other requests are served.
        """
        while True:
