...
Resolves the database URL from application settings so migrations always
target the same database the app uses:

    DATABASE_URL env var  →  else sqlite under DATA_DIR (same logic as config.py)
...
# Ensure backend/ is importable when invoked from backend/ directory.
import sys
...
import models  # noqa: F401,E402  — registers every ORM model on Base.metadata
