  ...
  return (
    <div className="min-h-full space-y-5">
      ...
      {loading ? (
        ...
        <>
          {/* ── Main grid: Sovereignty + Health + Resources ────── */}
                ...
                {status?.services && Object.entries(status.services).map(([name, svc]) => (
                  <div key={name} className="flex items-center justify-between p-2.5 bg-surface-overlay rounded-lg border border-surface-border">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[10px] font-medium ${svc.status === 'up' ? 'text-success-500' : 'text-danger-500'}`}>
                ...
                <div className="pt-2 border-t border-surface-border">
                  ...
                  {status?.models_loaded && status.models_loaded.length > 0 ? (
                    <div className="space-y-1.5">
                      {status.models_loaded.map((m, i) => (
                        <div key={i} className="flex items-center gap-2 px-2 py-1 bg-surface-overlay rounded-md">
                          <span className="w-1.5 h-1.5 rounded-full bg-success-500 animate-pulse-cyan" />
                          <span className="text-[11px] text-neutral-300 truncate">{m}</span>
                          <span className="ml-auto text-[9px] text-success-500 font-medium">Ready</span>
                        </div>
                      ))}
                    </div>
                  ) : (
            ...
            <div className="space-y-4">
              ...
              <div className="bg-surface-raised border border-surface-border rounded-xl p-5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Link to="/chat" className="flex items-center justify-center gap-2 px-3 py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg text-xs font-medium transition-colors">
                    New Chat
                  </Link>
                  <Link to="/knowledge" className="flex items-center justify-center gap-2 px-3 py-2.5 bg-surface-overlay hover:bg-surface-muted text-neutral-300 rounded-lg text-xs font-medium transition-colors border border-surface-border">
                    Knowledge Bases
                  </Link>
                  <Link to="/models" className="flex items-center justify-center gap-2 px-3 py-2.5 bg-surface-overlay hover:bg-surface-muted text-neutral-300 rounded-lg text-xs font-medium transition-colors border border-surface-border">
                    Browse Models
                  </Link>
                  <Link to="/providers" className="flex items-center justify-center gap-2 px-3 py-2.5 bg-surface-overlay hover:bg-surface-muted text-neutral-300 rounded-lg text-xs font-medium transition-colors border border-surface-border">
                    Providers
                  </Link>
                </div>
              </div>
              ...
              <div className="bg-surface-raised border border-surface-border rounded-xl p-5">
                ...
                <LiveLineChart
                  lines={[
                    { data: [cpuCurrent], color: '#22d3ee', label: 'CPU' },
                    { data: [ramCurrent], color: '#5a7fd9', label: 'RAM' },
                  ]}
                />
              </div>
            </div>
