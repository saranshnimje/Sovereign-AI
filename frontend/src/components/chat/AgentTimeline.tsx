...
function eventSummary(event: AgentEvent): string {
  ...
  return value === undefined ? '' : String(value).replace(/\s+/g, ' ').slice(0, 120)
}
...
export default function AgentTimeline({ events, isStreaming }: AgentTimelineProps) {
  ...
  // Token and low-level state events are intentionally hidden, while all
  // actionable lifecycle/tool events remain visible like an execution log.
  const timelineEvents = useMemo(() => events
    .filter(e => !['token', 'agent_state'].includes(e.event_type))
    .sort((a, b) => a.sequence - b.sequence), [events])

  // Automatically open the live execution log while the agent is working.
  useEffect(() => {
  ...
  return (
    <div className="rounded-lg border border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800/50 overflow-hidden mb-3">
      <button
        ...
        className="w-full flex items-center justify-between px-3 py-2 text-xs font-medium text-neutral-600 dark:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-700/50 transition-colors"
      >
        ...
        <div className="border-t border-neutral-200 dark:border-neutral-700 max-h-96 overflow-y-auto">
          {timelineEvents.map(event => {
            ...
            const displayCfg = event.event_type === 'done'
              ? {
                  ...cfg,
                  icon: failedDone ? '✕' : event.payload?.state === 'cancelled' ? '■' : '✓',
                  color: failedDone ? 'text-red-400' : event.payload?.state === 'cancelled' ? 'text-neutral-400' : 'text-emerald-400',
                  label: failedDone ? (event.payload?.state === 'timed_out' ? 'Timed out' : 'Failed') : event.payload?.state === 'cancelled' ? 'Cancelled' : 'Completed',
                }
              : cfg
              ...
              <div key={event.id}>
                ...
                {isEventExpanded && (
                  <div className="px-3 pb-2 pt-1 text-[10px] bg-neutral-100/50 dark:bg-neutral-700/30 mx-3 mb-1 rounded">
                    <pre className="text-neutral-500 dark:text-neutral-400 whitespace-pre-wrap break-words max-h-48 overflow-y-auto">
